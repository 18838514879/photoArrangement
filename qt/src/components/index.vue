<template>
  <div class="index" @click="isActiveItem = ''">
    <div class="handleList" :class="btnIsShow?'':'fadeAway'">
      ...
      <!-- <div class="btn addbtn" @click="updateColor"><input @click="stopBubbling" v-model="color" style="height:100%;border:none;outline:none;width:120px;"/>修改颜色</div> -->
  ...
  <!-- <input type="text"  v-if="$store.state.showFooter" /> -->
  </div>
</template>
...
export default {
  ...
  methods:{
    getList(){
      ...
      this.$axios({
        method: 'get',
        url:this.$baseurl+'/getItemList?userId=' + this.userId
      }).then((res)=>{
        if(res.data.code == 1){
